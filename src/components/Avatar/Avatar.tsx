...
*/
  size?: number | 'large' | 'small' | 'default' | { xs: number; sm: number; md: number; lg: number }
  /**
...
export default function Avatar({
  ...
  return (
    <div className={`avatar`}>
      <div className={`avatar-size-${size} avatar-gap-${gap} avatar-${shape} bg-gray-300`}>
        {src ? <img src={`${src}`} alt={`${alt}` || 'avater'} crossOrigin={crossOrigin} /> : icon}
      </div>
    </div>
  )
}
